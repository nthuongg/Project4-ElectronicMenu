...
// Thay đổi số lượng món ăn trong order hoặc xóa nếu quantity = 0
export const handleQuantityChange = (orderedItems, id, quantity) => {
    if (quantity === 0) {
        // Xóa món khỏi giỏ hàng nếu quantity là 0
        return orderedItems.filter((orderedItem) => orderedItem.id !== id);
    } else {
...
export const removeItemFromOrder = (orderedItems, id) => {
    return orderedItems.filter((orderedItem) => orderedItem.id !== id);
};